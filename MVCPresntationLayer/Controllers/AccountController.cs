            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    if (userMgr.FindUser(model.Email)){
                        ...
                        var oldUser = userMgr.AuthenticateUserByEmail(model.Email, model.Password);
                        var user = new ApplicationUser()
                        {
                            // populate these fields with existing data from oldUser
                            CharacterName = oldUser.CharacterName,
                            VillagerID = oldUser.VillagerID,
                            UserName = oldUser.LoginName,
                    ...
                    else
                    {
                        ...
                        //UserManager.AddToRole(user.Id, "Villager");
                        var result = await UserManager.CreateAsync(user, model.Password);
                        if (result.Succeeded)
                        {
                            await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
